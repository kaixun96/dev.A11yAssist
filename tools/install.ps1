...
param(
    [Parameter(Mandatory)]
    [ValidateSet('a11y-intake','a11y-file-bug','a11y-report','a11y-capture','a11y-validate','a11y-publish','a11y-workflow','a11y-knowledge','a11y-knowledge-odsp','a11y-bug-bash','a11y-test-categories','a11y-setup','all')]
    [string]$Plugin,
    [switch]$Execute,
    [switch]$WithAgentOW
)
...
$names = if ($Plugin -eq 'all') {
    @('a11y-intake','a11y-file-bug','a11y-report','a11y-capture','a11y-validate','a11y-publish','a11y-workflow','a11y-knowledge','a11y-bug-bash','a11y-test-categories','a11y-setup')
} else { @($Plugin) }
...
if ($Plugin -in @('a11y-knowledge','a11y-knowledge-odsp')) {
    ...
    Write-Output 'Install a11y-test-categories separately and configure pluginRoots.testCategories. Restart Copilot before using /a11y-bug-bash. Knowledge/setup modules are included; host preparation and Bug filing require separate authorization. Final reporting uses the shared a11y-report implementation.'
    ...
    Write-Output 'Restart Copilot before using /a11y-setup. Establish original DevBox resource/setup ownership before preparing selected tools. Resource MCP calls require A11Y_ASSIST_CONFIG; local inventory uses approved host tools. No AgentOW prerequisite.'
