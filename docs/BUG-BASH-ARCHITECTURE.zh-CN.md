...
修改时同步维护中英文版本。本文说明插件是什么、有哪些子能力、如何组合，以及用户的输入和最终输出。基线：0.19.0 版本，2026-09-14；源码编排链路已实现，部署和现场验收另行进行。实际可执行范围由受支持适配器决定，不承诺任意浏览器/AT 组合都已可用。
...
**尚未提供通用内置实现：** NVDA/Narrator/Voice Access 采集驱动、登录态/服务端事务、
扫描器/视觉测量、云资源恢复和凭据代理。它们已有类型明确的接入契约，但仍需要实际部署
提供对应实现并验收。不能把编排与提单源码当成这些驱动已经实现，也不能把矩阵行齐全当成
真实场景全部执行完成。
...
随包提供的原生浏览器模块限定于获批的匿名、客户端 HTTPS 场景。已认证事务、扫描器、视觉测量及真实具名 AT 需要明确兼容的部署适配器；缺失时报告缺口，不以 DOM 证据替代，也不悄悄算作完成。
