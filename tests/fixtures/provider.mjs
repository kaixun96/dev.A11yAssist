...
if (request.operation === 'status') {
  ...
  const receipt = {
    ...identity, stage: request.stage, outcome: request.input.outcome ?? 'pass',
    reason: request.input.outcome ? 'Test fixture outcome' : undefined,
    gates: Object.fromEntries(contract.stages.find(s => s.id === request.stage).gates.map(g => [g, true])),
    ...
    entrypoint: '/agentow-a11y', model: 'gpt-6-astra', prCreated: false,
    beforeReceiptSha256: request.run.receipts.find(r => r.stage === 'before')?.sha256,
