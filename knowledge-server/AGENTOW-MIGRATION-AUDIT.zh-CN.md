...
覆盖截至：2026-09-15；源清单审计：2026-09-14。源：[kaixun96/dev.AgentOW，固定提交 7896845e51d75b0b9d632a2fd61876bc2f556ea5](https://github.com/kaixun96/dev.AgentOW/tree/7896845e51d75b0b9d632a2fd61876bc2f556ea5)。目标为 authored 三包 `0.1.1`、已协调的精确依赖及服务 `0.1.0`。本地链接指向已实现正文；消费方实际读取的快照由生成引用和哈希另行标识。
...
2026-09-15 验证基线：**KB 104 项、现有插件 79 项测试通过**，两套生成检查通过。
当前快照包含 35 个条目，其中七个经人工策划的条目带有标签；先前不可变快照继续保留。
MCP 回归覆盖迁入规则的发现、检索和完整读取。

内容维护见[扩展指南](TECH-DESIGN.zh-CN.md#32-扩展已迁移规则)；版本、兼容性、生成与发布见
[发布政策](TECH-DESIGN.zh-CN.md#9-版本生成与发布)。第 2 节记录固定来源的归档检查方法。
本地检查不证明 live AT 行为或官方来源批准。
