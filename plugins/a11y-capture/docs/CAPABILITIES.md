...
All ten plugins provide their own read-only knowledge MCP selecting the 32 current
Common, Fluent and SharePoint entries. Knowledge requires Node.js 22+ and enabled MCP, not
providers, `A11Y_ASSIST_CONFIG` or a peer plugin. Knowledge and Bug Bash ship
only the two knowledge runtime files, no operational MCP. Bug Bash page/AT
checks use separately authorized host tools and ownership gates; see [Bug Bash](BUG-BASH.md).
...
The current [evidence-v1 validator](../src/runtime/evidence-v1.mjs) defines the accepted
