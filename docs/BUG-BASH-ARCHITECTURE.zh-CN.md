...
修改时同步维护中英文版本。本文说明插件是什么、有哪些子能力、如何组合，以及用户的输入和最终输出。架构基线于 2026-09-14 随 v0.20.0 引入，本文持续维护当前行为；已安装版本以 `plugin.json` 为准，不要把历史基线当作当前包版本。源码编排链路已实现，部署和现场验收另行进行。实际可执行范围由受支持适配器决定，不承诺任意浏览器/AT 组合都已可用。
