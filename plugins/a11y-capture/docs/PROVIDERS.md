...
Bundled portable knowledge needs no MCP, providers or `A11Y_ASSIST_CONFIG`.
Knowledge and Bug Bash carry no executable runtime or MCP server; Bug Bash page/AT
checks require separately authorized host tools and owned resources. The
configuration below is for execution operations, not knowledge or discovery
installation. Setup likewise has no MCP; its scoped native Windows script and
dependency templates use separately authorized host tools, not this provider
protocol. Installation is neither live readiness nor execution authority.
