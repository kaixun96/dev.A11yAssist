...
const sourceKb = join(repository, 'accessibility-kb');
const knowledgePlugin = 'a11y-knowledge';
...
async function fixture(t, name = knowledgePlugin) {
  ...
  const install = async pluginName => {
    const plugin = join(dir, 'installed', pluginName);
    await cp(join(repository, 'plugins', pluginName), plugin, { recursive: true });
    return plugin;
  };
  const plugin = await install(name);
  const reference = await readJson(join(plugin, 'references/knowledge.json'));
  ...
  return { dir, cache, cacheFile, plugin, reference, bytes, install, env: { A11Y_ASSIST_KB_CACHE_ROOT: cache } };
}
...
async function repinRaw(plugin, reference, bytes) {
  const pinned = structuredClone(reference);
  pinned.distribution.sha256 = digest(bytes);
  validateKnowledgeReference(pinned);
  await writeFile(join(plugin, 'references/knowledge.json'), json(pinned));
  return pinned;
}

test('single installed plugin cold-downloads pinned bytes, then revalidates offline without configuration', async t => {
  const f = await fixture(t);
  assert.equal(await findDevelopmentKbRoot(f.plugin), null);
  assert.deepEqual(await directoryEntries(join(f.dir, 'installed')), [knowledgePlugin]);
  assert.deepEqual(await directoryEntries(join(f.plugin, 'accessibility-kb')), []);
  // Exercise the copied standalone module too: it cannot import repository helpers.
  const installedRuntime = await import(pathToFileURL(join(f.plugin, 'runtime/knowledge.mjs')).href);
  ...
  const downloaded = await installedRuntime.loadKnowledgeSnapshot(f.plugin, { env, fetchImpl: http.fetchImpl });
  ...
  const cached = await installedRuntime.loadKnowledgeSnapshot(f.plugin, { env, fetchImpl: offline.fetchImpl });
  assert.equal(cached.origin, 'cache');
  assertSnapshot(cached, f.reference, f.bytes);
  assert.equal(offline.calls.length, 0);
});

test('unified knowledge and execution plugins share cache while a synthetic Common consumer stays isolated', async t => {
  const f = await fixture(t);
  const second = await f.install('a11y-capture');
  ...
  const http = syntheticHttp(f.reference, () => new Response(f.bytes));
  await loadKnowledgeSnapshot(f.plugin, { env: f.env, fetchImpl: http.fetchImpl });
  const offline = offlineHttp();
...
test('concurrent cold downloads publish atomically and remove only their own temporary files', { timeout: 15000 }, async t => {
  ...
  const settled = await Promise.allSettled([f.plugin, second].map(plugin =>
    loadKnowledgeSnapshot(plugin, { env: f.env, fetchImpl: http.fetchImpl })));
...
test('an invalid concurrent cache winner is rejected without replacing or deleting it', async t => {
  ...
  await assert.rejects(loadKnowledgeSnapshot(f.plugin, { env: f.env, fetchImpl: http.fetchImpl }), /Invalid KB cache.*not overwritten/);
  assert.equal(http.calls.length, 1);
...
test('cache tampering is revalidated on every request and never fetched or repaired', async t => {
  ...
  for (let request = 0; request < 2; request++) {
    await assert.rejects(loadKnowledgeSnapshot(f.plugin, { env: f.env, fetchImpl: http.fetchImpl }),
...
test('configured roots and validated development loads match the downloaded selected snapshot', async t => {
  for (const name of [knowledgePlugin, 'a11y-capture', 'a11y-bug-bash', 'a11y-setup']) await t.test(name, async t => {
    const f = await fixture(t, name);
    const local = join(f.dir, 'shared-kb');
    await cp(sourceKb, local, { recursive: true });
    const offline = offlineHttp();
    const configured = await loadKnowledgeSnapshot(f.plugin, { kbRoot: local, env: f.env, fetchImpl: offline.fetchImpl });
    const environment = await loadKnowledgeSnapshot(f.plugin, {
    ...
    const priority = await loadKnowledgeSnapshot(f.plugin, {
    ...
    const devPlugin = join(repository, 'plugins', name);
    assert.equal(await findDevelopmentKbRoot(devPlugin), sourceKb);
    const development = await loadKnowledgeSnapshot(devPlugin, { env: f.env, fetchImpl: offline.fetchImpl });
...
test('development detection requires repository identity and missing development KB does not download', async t => {
  const f = await fixture(t);
  const checkout = join(f.dir, 'checkout');
  const plugin = join(checkout, 'plugins', knowledgePlugin);
  await cp(f.plugin, plugin, { recursive: true });
  assert.equal(await findDevelopmentKbRoot(plugin), null, 'A plugins/name shape alone is not a checkout');
  for (const path of ['package.json', 'src/contracts/plugins.json', 'src/catalog.json', '.github/plugin/marketplace.json', 'tools/build.mjs']) {
  ...
  assert.equal(await findDevelopmentKbRoot(plugin), missingKb);
  const catalogPath = join(checkout, 'src/catalog.json');
  const catalog = await readJson(catalogPath);
  for (const changed of [catalog.filter(entry => entry.name !== knowledgePlugin),
    [...catalog, catalog.find(entry => entry.name === knowledgePlugin)]]) {
    await writeFile(catalogPath, json(changed));
    assert.equal(await findDevelopmentKbRoot(plugin), null, 'Catalog identity must be present and unique');
  }
  await writeFile(catalogPath, json(catalog));
  const marketplacePath = join(checkout, '.github/plugin/marketplace.json');
  const marketplace = await readJson(marketplacePath);
  const changed = structuredClone(marketplace);
  changed.plugins.find(entry => entry.name === knowledgePlugin).source = './elsewhere';
  await writeFile(marketplacePath, json(changed));
  assert.equal(await findDevelopmentKbRoot(plugin), null, 'Neutral marketplace must name the exact development plugin path');
  await writeFile(marketplacePath, json(marketplace));
  assert.equal(await findDevelopmentKbRoot(plugin), missingKb);
  const offline = offlineHttp();
  await assert.rejects(loadKnowledgeSnapshot(plugin, { env: f.env, fetchImpl: offline.fetchImpl }), /Missing KB directory/);
  assert.equal(offline.calls.length, 0);
  await cp(sourceKb, missingKb, { recursive: true });
  const valid = await loadKnowledgeSnapshot(plugin, { env: f.env, fetchImpl: offline.fetchImpl });
  ...
  await assert.rejects(loadKnowledgeSnapshot(plugin, { env: f.env, fetchImpl: offline.fetchImpl }), /Local KB file SHA-256 mismatch/);
  assert.equal(offline.calls.length, 0);
  assert.deepEqual(await directoryEntries(f.cache), []);
});
...
test('invalid explicit roots never fall back to environment, development, a warm cache, or HTTP', async t => {
  ...
  const offline = offlineHttp();
  for (const plugin of [f.plugin, join(repository, 'plugins', knowledgePlugin)]) {
    for (const kbRoot of ['', 'relative/kb', null, join(f.dir, 'missing-kb'), fileRoot, corruptRoot]) {
      await assert.rejects(loadKnowledgeSnapshot(plugin, {
...
test('invalid environment KB/cache roots fail closed without network or cache changes', async t => {
  ...
  for (const key of ['A11Y_ASSIST_KB_ROOT', 'A11Y_ASSIST_KB_CACHE_ROOT']) {
    for (const value of invalid) {
      await assert.rejects(loadKnowledgeSnapshot(f.plugin, {
...
test('HTTP and raw-byte failures never publish a cache artifact', async t => {
  ...
  for (const [name, respond, expected] of cases) await t.test(name, async t => {
    ...
    await assert.rejects(loadKnowledgeSnapshot(f.plugin, { env: f.env, fetchImpl: http.fetchImpl }), expected);
    assert.equal(http.calls.length, 1);
...
test('repinned synthetic malformed artifacts reach deep validation but are never cached', async t => {
  ...
  for (const [name, mutate, expected] of cases) await t.test(name, async t => {
    ...
    const reference = await repinRaw(f.plugin, f.reference, bytes);
...
test('valid hashes cannot bypass semantic validation of selected package contents', async t => {
  ...
  const pinned = await repinRaw(f.plugin, reference, bytes);
  const http = syntheticHttp(pinned, () => new Response(bytes));
  await assert.rejects(loadKnowledgeSnapshot(f.plugin, { env: f.env, fetchImpl: http.fetchImpl }), /Unknown KB source/);
  assert.equal(http.calls.length, 1);
  assert.deepEqual(await directoryEntries(f.cache), []);
});
...
test('reference validation rejects unpinned selectors and caller-controlled distribution endpoints before fetching', async t => {
  ...
  for (const reference of invalid) {
    ...
    await writeFile(join(f.plugin, 'references/knowledge.json'), json(reference));
    await assert.rejects(loadKnowledgeSnapshot(f.plugin, { env: f.env, fetchImpl: offline.fetchImpl }), /Invalid|Unsupported/);
  }
  assert.equal(offline.calls.length, 0);
  assert.deepEqual(await directoryEntries(f.cache), []);
});