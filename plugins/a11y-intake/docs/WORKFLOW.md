...
Keep the original run identity and authoritative claim throughout execution;
never create a competing claim or silently switch an active run's implementation.
...
Source operations use the configured source connection and an owned worktree.
Installation does not authorize machine access or establish evaluator readiness.
...
|---|---|
| intake (0-1) | Atomically claim the exact Bug; inspect complete comments then attachments; define acceptance and seal scenario. No branch, evaluator or source worktree acquired early. |
...
| source (3-5) | Caller-selected source connection, owned worktree and verified executor. Minimal implementation, targeted validation, exact HEAD and loaded affected resource. No PR yet. |
...
| cleanup (10) | Stop owned AT/browser/recorders, restore audio, close debug ports, preserve artifacts, release source worktree then evaluator then work-item claim with the applicable ownership proof, write insights and deliver owner summary. |
...
There is no unverified-PR fallback. The workflow APIs enforce this order against
the shared run. Independent
...
## Current workflow APIs and version pinning

Create/status/execute/reconcile/progress/abandon are current full-workflow APIs,
not prerequisites for independent capability calls. Qualify configured providers
before production use; never execute competing implementations on one Bug to
compare side effects.

Record effective package/provider versions and hashes in receipts. Refresh only
at a safe point, never silently during an active run. Any upgrade that changes
capture semantics invalidates affected evidence until rerun.
