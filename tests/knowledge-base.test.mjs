...
test('shared KB validates all entries and binds its complete manifest to release', async () => {
  ...
  assert.equal(release.sharedKnowledge.sha256, digest(bundle.files.get('manifest.json')));
  assert.deepEqual(release.sharedKnowledge.packages, bundle.manifest.packages);
...
test('current KB has no archive metadata or routing', async () => {
  const kb = await loadKnowledgeBase(kbRoot);
