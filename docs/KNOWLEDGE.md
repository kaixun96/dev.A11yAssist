...
- MAS/WCAG source lookup follows [Liquid standards retrieval](LIQUID-STANDARDS.md)
  through the caller's authenticated MCP connection. Knowledge and test-category
  packages (including Bug Bash's internal modules) bundle the same instructions.
  Root plugin manifests and `.mcp.json` declare the remote Liquid HTTP server with
  four read-only tools; bundled modules do not register it again. No token is
  shipped and no internal standards bodies are mirrored. Offline guidance remains usable without this optional
  connection; missing official sources are reported explicitly.
...
- `a11y-knowledge` is an independently installable read-only skill, without a bundled MCP server,
...
- `a11y-knowledge-odsp` remains an independently installable compatibility package
  ...
  Both knowledge packages have no bundled MCP server, providers or executable tools and do not
