...
export async function findDevelopmentKbRoot(pluginRoot) {
  try {
    const plugin = absolutePath(pluginRoot, 'pluginRoot');
    assert.equal(basename(dirname(plugin)), 'plugins');
    const repository = dirname(dirname(plugin));
    assert.equal((await readJson(repository, 'package.json')).name, '@a11y-assist/plugins');
    const manifest = await readJson(plugin, 'plugin.json');
    assert.equal(manifest.name, basename(plugin));
    const contract = await readJson(repository, 'src/contracts/plugins.json');
    object(contract, 'plugin contract');
    assert([...Object.keys(contract), 'a11y-knowledge', 'a11y-bug-bash', 'a11y-setup'].includes(manifest.name));
    const catalog = await readJson(repository, 'src/catalog.json');
    assert(Array.isArray(catalog) && catalog.filter(entry => entry?.name === manifest.name).length === 1);
    const marketplace = await readJson(repository, '.github/plugin/marketplace.json');
    assert(Array.isArray(marketplace.plugins) && marketplace.plugins.some(entry =>
      entry?.name === manifest.name && entry.source === `./plugins/${manifest.name}`));
    await fileStat(repository, 'tools/build.mjs');
    // Repository identity, not KB existence, chooses development mode. A missing
    // or corrupt KB in a validated checkout must fail, not trigger a download.
...
/**
 * Load a fresh, verified selected snapshot; no process-global body cache.
...
export async function loadKnowledgeSnapshot(pluginRoot, { kbRoot, env = process.env, fetchImpl = globalThis.fetch } = {}) {
  pluginRoot = absolutePath(pluginRoot, 'pluginRoot');
