...
`a11y_knowledge_knowledge_list`, select Common foundations and the applicable
Fluent/SharePoint entry IDs for the actual component stack, then
call `a11y_knowledge_knowledge_read(id)` for their actual content. Start with
foundations and read only relevant complete topics and find/fix/prevent/design/test
procedures. `a11y_knowledge_knowledge_search(query)` can narrow selection, but a
