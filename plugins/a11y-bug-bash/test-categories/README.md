...
The original nine procedure documents are copied without content changes from
...
When Voice Access is requested, use `voice-access.md` for its execution steps
and evidence requirements; the short note in `touch-pointer.md` is not a complete
Voice Access procedure. Keep voice-control results separate from screen-reader
speech and from pointer-only observations.
