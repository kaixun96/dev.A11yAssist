...
async function developmentKbRoot(pluginRoot) {
  ...
  if (!candidate) {
    throw new Error('Shared KB is not configured. The host must provide an absolute kbRoot or A11Y_ASSIST_KB_ROOT; development lookup only supports this repository plugin layout.');
  }
  return candidate;
}

// Host-side setup/read helper. It is not shipped into knowledge plugins and is
// not an instruction for read-only skills to execute shell commands.
export async function resolveKnowledgeReference(pluginRoot, { kbRoot, env = process.env } = {}) {
  assert(typeof pluginRoot === 'string' && isAbsolute(pluginRoot), 'pluginRoot must be absolute');
  const canonicalPluginRoot = await realpath(pluginRoot);
  const reference = await load(join(canonicalPluginRoot, 'references/knowledge.json'));
  ...
  if (configured !== undefined) {
    ...
    location = configured;
  } else location = await developmentKbRoot(canonicalPluginRoot);
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    assert(process.argv.length >= 3 && process.argv.length <= 4,
      'Usage: node tools/knowledge-reference.mjs <absolute-plugin-root> [absolute-kb-root]');
