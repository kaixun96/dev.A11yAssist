...
`a11y-test-categories` 是可独立安装的测试规程插件，为每个测试对象及状态规定检查内容、执行顺序和所需证据。包含九类：键盘与焦点、屏幕阅读器、结构语义、方向与输入用途、视觉颜色、时间与动画、动态内容、触摸指针、认证与表单。
...
调用方先列出对象及状态；本插件为每一项展开全部九类的每个编号步骤。浏览器工具执行页面操作，真实 AT/capture 工具提供实际辅助技术证据。调用方判断问题并交付报告。本插件防止漏掉矩阵行却宣称完整覆盖，不自动判断 WCAG。
...
每个对象/状态都展开九类中的每个编号步骤，适用项按顺序实际执行。不适用必须给出针对该对象的具体理由；缺少 AT、时间或权限属于缺口，不是不适用。同一证据只有实际覆盖每个对象/状态/步骤时才能复用；无需为增加计数机械重复全局操作。必须阅读完整规程，包括编号步骤之后的证据和清理要求。九类不是完整标准，仍需补充 feature 特有的检查。
...
- [规程索引及固定版本来源](../src/test-categories/procedures/README.md)（打包时自动转为安装包内路径）。
