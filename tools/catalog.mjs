...
const referenceTitles = {
  ...
  'modules/a11y-knowledge/knowledge/README.md': { en: 'Included portable knowledge', zh: '随包通用知识' },
...
const labels = {
  en: {
    ...
    advice: 'For knowledge, install **a11y-knowledge** once: offline portable topics support generic and ODSP source review with supplied current component documentation. Choose **a11y-workflow** only if you want the complete workflow.',
    ...
    developmentText: 'Users can stay in the catalog and plugin pages. [Development](docs/DEVELOPMENT.md) explains source and packaging; [release guidance](docs/RELEASING.md) covers maintainer validation.',
  ...
  zh: {
    ...
    advice: '知识只需安装 **a11y-knowledge**：可离线使用的通用主题结合提供的当前组件文档，支持通用和 ODSP 源码审查。只有需要整套流程时才选 **a11y-workflow**。',
    ...
    development: '维护者入口',
    developmentText: '普通用户只需看插件目录和插件页面。[开发说明](docs/DEVELOPMENT.md) 解释源码及打包；[发布说明](docs/RELEASING.md) 描述维护者验证流程。',
    license: '访问与许可',
